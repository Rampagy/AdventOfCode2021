            ...
            if v in graph:
                for search_v in graph[v]:
                    ...
                    if search_v.isupper() or \
                            (search_v not in node_count and search_v.islower()) or \
                            (search_v in node_count and node_count[search_v]  < 1 and search_v.islower()):
                        q.put([search_v])
...
def BFS_Path(parent, path, start, path_set):
    ...
    if state != start:
        for p in parent[state]:
            if p.isupper() or (p.islower() and p not in path):
